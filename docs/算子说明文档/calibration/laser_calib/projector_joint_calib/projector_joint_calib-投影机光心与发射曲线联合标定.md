...
| 严重程度 | 风险描述 | 影响 | 实验数据 |
|:--------:|---------|------|---------|
| 🟡 中 | t_z 信号弱（t_true.z≈3mm 光心几乎在相机平面） | t_z 精度高度 seed 依赖 | 5 seed@σ=0.2 范围 0.06~3.34mm，均值 0.90mm |
| 🟡 中 | 激光线截断（两端不可见）增大 t_z 方差 | 完整→截断均值 0.90→1.54mm（1.7×） | tx/ty 不受影响 |
| 🟡 中 | t_z ≈ k·σ²（k均值≈22，范围 7~43） | t_z 是噪声二阶效应，非算法 bug | Ceres交叉验证一致，确认是物理极限 |
...
**精度特征（5 seed 均值，真实硬件参数 t_true=(80,3,3), f=1500px）**：
...
| 0.10mm | — | — | 0.13mm | — | 中等噪声 |
| 0.15mm | — | — | 0.24mm | — | — |
| 0.20mm（产线） | **0.15mm** | **0.01mm** | **0.90mm** | 0.29~1.74mm | 真实硬件，5 seed |
| 0.20mm+截断 | 0.16mm | 0.01mm | **1.54mm** | 0.06~3.34mm | 激光线两端缺5~20% |

> - t_z ≈ k × σ²（k 均值≈22，范围 7~43，seed 依赖）。降 σ 一半 → t_z 降 4 倍（平方级杠杆）。
