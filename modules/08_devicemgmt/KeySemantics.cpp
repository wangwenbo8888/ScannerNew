...
// 测试 test_key_semantics.cpp 逐条钉死；260831 批1-C：输入换 serial::GestureEvent
// ——手势判定归 MCU G01，本类只做语义映射，转移表逻辑零改动）
// ============================================================================
...
void KeySemantics::onGesture(const serial::GestureEvent& ev, const MenuState& menu) {
    using K = serial::KeyId;
    using G = serial::Gesture;          // 1/2/3 = Short/Double/Hold（MCU 已判）
    ...
    const bool startStop = ev.key == K::Middle && ev.gesture == G::Short && menu.layer == 1;
    if (!startStop && !gate_()) {
        actions_.dropped("门禁");
        return;
    }
    ...
    case K::Right:
        ...
        if (menu.layer == 2) {                                // 菜单优先于调节（口径钉死）
            if (ev.key == K::Left) actions_.cursorLeft();
            else actions_.cursorRight();
        } else if (menu.adjustCtx != MenuState::AdjustCtx::None) {
            if (ev.key == K::Left) actions_.adjustDown();
