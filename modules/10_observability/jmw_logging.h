...
// （§8.2 格式纪律）。
// 源位置：宏族在调用点展开为 SPDLOG_*——file:line 由 pattern 的 %s:%# 自动
// 附带（无需手动传 __FILE__）；散装 spdlog::info 等函数调用不带源位置。
...
#define JMW_LOG_DEBUG(tag, ...) SPDLOG_DEBUG("[{}] {}", tag, fmt::format(__VA_ARGS__))
