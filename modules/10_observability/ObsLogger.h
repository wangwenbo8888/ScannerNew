#pragma once
// ============================================================================
// ObsLogger.h — 统一日志装配（控制台 + 按日文件）+ 排障包（设计方案 §5）
//
//   · obsLoggerInit：stdout_color + daily_file 双 sink 组 spdlog default
//     logger（pattern "[%H:%M:%S.%e][%l][%n][%s:%#] %v"——含源文件名:行号，
//     由 JMW_LOG_* 宏族在调用点自动捕获；logger 名 jmw，level info）；
//     文件按日命名 jmw_YYYY-MM-DD.log，跨零点自动切换（运行中生效）；
//     超保留份数自动删最旧（sink 构造与轮转时均清理）。main 最先装配
//     （§7.3：ObsLogger → CrashHandler → 其余件）。幂等——重复调用先拆旧
//     装配再重建（旧 sink 文件句柄随之释放）
//   · obsLoggerShutdown：flush 余量 + drop 全部 logger/sink——Windows 下删除
//     日志目录前必须先调（daily sink 持有当日文件句柄）
...
struct ObsLoggerConfig {
    ...
    size_t maxFiles = 7;                    // 按日保留份数（含当天；超限删最旧）
