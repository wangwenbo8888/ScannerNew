...
void FuseConsumer::processOne(FrameResult& fr) {
    ...
    // 3) 攒观测：R_init/T_init 逐帧保留；markerObs 查高精度集；激光 host 拷贝入缓存
    FrameObs fo;
    ...
    // 4) 渲染节流：第 N 帧推送一次（激光句柄由 ILaserFuse 适配器持有 → nullptr）
    const uint64_t n = consumed_.fetch_add(1, std::memory_order_relaxed) + 1;
    if (deps_.sceneFeed &&
