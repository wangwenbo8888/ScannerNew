...
> 快速查阅。详细架构见 `docs/architecture/software-architecture-skeleton-design.md`，`factory_calib/` 的 AI 规则见 `factory_calib/AGENTS.md`（只读保护区）。
...
├── 算子说明文档/         # 45 份算子/渲染组件说明（A–K 模板），自带索引 算子目录.md，
│                        #   分 core/(19) calibration/(17) scanning/(6) display/(3)
└── 开发需要的信息/       # 业务流详细设计稿（标定/扫描/姿态）+ 相机
    └── 业务流/
```
...
| `模块功能细化.md` | 各模块功能细化（双目17线激光扫描仪，2026-07-12） |
| `算子规范.md` | 算子契约 + 工程集成规范（v2.0，§0–§7） |
| `算子目录.md` | 算子说明文档目录索引（导航至 `docs/算子说明文档/` 下 45 份说明） |
