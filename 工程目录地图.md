...
> 快速查阅。详细架构见 `架构设计-简洁版.md`，`factory_calib/` 的 AI 规则见 `factory_calib/AGENTS.md`（只读保护区）。**AI 会话接力 / 当前进度见 `开发进度.md`（默认限读：快照+关键决策+近 3 条日志）。**
...
├── 模块功能/            # 11 业务模块功能细化（01-11 各一文档），索引见根 模块功能细化.md
...
| `模块功能细化.md` | 模块功能索引/总览（11 模块已拆分至 `docs/模块功能/01~11-*.md`；含硬件表/基础设施/SDK/路线图） |
