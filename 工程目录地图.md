...
> 快速查阅。详细架构见 `docs/architecture/software-architecture-skeleton-design.md`，AI 索引见 `AGENTS.md`。
> **阶段**：骨架搭建中——framework 层契约桩 + 业务模块骨架；已迁入实现：`modules/09_operatorlib`（全部算子）+ `modules/03_rendering`（display 渲染组件）。
...
## framework/ — 层契约（9 层 + tests）

> 5 层架构的契约定义，目前为接口桩。子命名空间 `Scanner::<层>`。每层 `CMakeLists.txt` + 一个接口/契约头。
...
> 唯一有完整实现的模块。单静态库 `mod_operatorlib`（core+calibration+scanning 合建）。算子规范见 `算子规范.md`，逐算子说明见 `docs/算子说明文档/`。
...
```
docs/
├── architecture/        # software-architecture-skeleton-design.md（5 层架构设计）
├── plans/               # 2026-07-05 系列：skeleton-impl / operator-migration-impl /
│                        #   display-migration(-design) / dev-plan-tracking-design
├── 算子说明文档/         # 42 算子逐份说明（A–K 模板），自带索引 算子目录.md，分 core/ calibration/ scanning/ display/
├── 开发计划/             # （预留）
└── 开发需要的信息/       # 业务流文档（标定/扫描/姿态判断流水线方案）
    └── 业务流/
```
