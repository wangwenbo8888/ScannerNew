...
> **阶段**：骨架搭建中——framework 层契约桩 + 空模块；仅 `modules/09_operatorlib`（算子迁入）有完整实现。
...
├── modules/     # 11 业务模块（骨架桩），仅 09 有实现
├── sdk/         # 接入端 B（桩）
├── app/         # exe 入口（jeammware.exe）
├── docs/        # 架构 / 计划 / 算子说明文档
...
> 5 层架构的契约定义，目前为桩。子命名空间 `Scanner::<层>`。
...
| `algorithm/` | 算法层（调用算子库） |
| `service/` | 业务服务编排 |
| `workflow/` | 流水线/工作流编排（E 核调度核心） |
| `ui/` | UI 层（Qt，BUILD_UI=OFF 暂缓） |
| `infra/` | 基础设施（配置/调度/IPC 等） |
| `crosscut/` | 横切关注点（日志/监控/异常） |
| `tests/` | 框架冒烟测试 `test_smoke.cpp` |
...
├── plans/           # 骨架实施 / 算子迁移 计划
└── 算子说明文档/    # 42 算子逐份说明（A–K 模板），索引见 算子说明文档/算子目录.md
                     #   注：display/ 3 份（scanner_viewer 等）代码未迁入，待处理
```
